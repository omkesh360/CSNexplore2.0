...
<header class="sticky top-0 w-full z-50 glass-dark border-b border-white/5">
  ...
  <script>document.getElementById("mob-btn").addEventListener("click",function(){document.getElementById("mob-menu").classList.toggle("hidden");});</script>
</header>';
}
...
<div class="w-full bg-slate-100 border-b border-slate-200">
  <div class="max-w-4xl mx-auto px-4 py-3 flex items-center gap-2 text-sm text-slate-500 flex-wrap">
    <a href="../index.php" class="hover:text-[#ec5b13] transition-colors flex items-center gap-1"><span class="material-symbols-outlined text-base">home</span>Home</a>
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <a href="../blogs.php" class="hover:text-[#ec5b13] transition-colors">Blogs</a>
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <a href="../blogs.php?category='.urlencode($blog['category']).'" class="hover:text-[#ec5b13] transition-colors">'.htmlspecialchars($blog['category']).'</a>
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <span class="text-slate-700 font-semibold truncate max-w-xs">'.htmlspecialchars($blog['title']).'</span>
  </div>
</div>
<main class="bg-white min-h-screen">
  <div class="w-full h-[380px] md:h-[480px] relative overflow-hidden">
    <img src="'.htmlspecialchars($blog['image'] ?? '').'" alt="'.htmlspecialchars($blog['title']).'" class="w-full h-full object-cover" onerror="this.src=\'https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=1200&q=80\'"/>
...
<!-- Breadcrumb -->
<div class="w-full bg-slate-100 border-b border-slate-200">
  <div class="max-w-6xl mx-auto px-4 py-3 flex items-center gap-2 text-sm text-slate-500 flex-wrap">
    <a href="../index.php" class="hover:text-[#ec5b13] transition-colors flex items-center gap-1"><span class="material-symbols-outlined text-base">home</span>Home</a>
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <a href="../listing.php?type='.$type.'" class="hover:text-[#ec5b13] transition-colors">'.htmlspecialchars($meta['label']).'</a>
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <span class="text-slate-700 font-semibold">'.htmlspecialchars($item['name']).'</span>
  </div>
</div>
  ...
  <div class="max-w-6xl mx-auto px-4">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        <!-- Hero image -->
        <div class="rounded-2xl overflow-hidden h-72 md:h-96 shadow-lg">
          <img src="'.htmlspecialchars($item['image'] ?? '').'" alt="'.htmlspecialchars($item['name']).'" class="w-full h-full object-cover" onerror="this.src=\'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&q=80\'"/>
        </div>
        <!-- Title & rating -->
        <div class="bg-white rounded-2xl p-6 shadow-sm">
          <div class="flex items-start justify-between gap-4 mb-3">
            <div>
              <span class="text-xs font-bold text-[#ec5b13] uppercase tracking-widest mb-1 block">'.htmlspecialchars($meta['label']).'</span>
              <h1 class="text-2xl md:text-3xl font-serif font-black">'.htmlspecialchars($item['name']).'</h1>
            </div>
            <div class="flex items-center gap-1 bg-amber-50 px-3 py-1.5 rounded-xl shrink-0">
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm text-center">
          <p class="text-sm text-slate-500 mb-3">Need help? Call or WhatsApp us</p>
          <a href="tel:+918600968888" class="flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold py-3 px-4 rounded-xl text-sm transition-colors mb-2">
            <span class="material-symbols-outlined text-base text-[#ec5b13]">call</span>+91 86009 68888
          </a>
          <a href="https://wa.me/918600968888" class="flex items-center justify-center gap-2 bg-green-50 hover:bg-green-100 text-green-700 font-bold py-3 px-4 rounded-xl text-sm transition-colors">
